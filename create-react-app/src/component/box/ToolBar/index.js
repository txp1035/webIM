...
// import userImg from '@/assets/img/faces/ee_1.png'
...
class ToolBar extends Component {
  ...
  render() {
    const requireContext = require.context('@/assets/img/faces', true, /^\.\/.*\.png$/);
    ...
    const list = numbers.map((number, index) => {
      ...
      return <li key={index} style={{ background: url }} onClick={this.liClick} />;
    });
    return (
      <div className="tool-bar">
        <i className={this.props.name} onClick={this.iClick}>
          <ul style={{ display: this.state.ul }}>{list}</ul>
        </i>
      </div>
    );
  }
